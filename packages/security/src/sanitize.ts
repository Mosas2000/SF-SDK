 ...
 *
 * These utilities provide defense-in-depth by stripping potentially
 * dangerous content at the data layer before it ever reaches rendering.
 *
 * Use cases:
 * - Proposal titles and descriptions from on-chain storage
 * - Error messages from untrusted sources
 * - Any string that originated outside the application
 */
 ...
 * (U+000D) which are legitimate whitespace
 *
 * @param input - Raw string
...
/**
 * Truncate a sanitized string to a maximum length with ellipsis
 *
 * The total output length including the ellipsis will not exceed maxLength
 *
 ...
 * cannot break URL parsing or inject additional path segments
 *
...
/**
 * Validate that a string looks like a valid Stacks transaction ID
 *
...
/**
 * Sanitize URL to prevent protocol injection attacks
 *
...
export function sanitizeUrl(
  ...
  try {
    ...
    // If URL parsing fails, check for protocol-like patterns
